...
{
    public class BitFuncParseRespCommandImplTests
    {
        [Fact]
        public void Simple()
        {
            foreach (var cmd in Enum.GetValues<RespCommand>())
            {
                ...
                var parsedUpper = BitFuncParseRespCommandImpl.Calculate(ref respUpperBytes.AsSpan().EndRef(), ref respUpperBytes.AsSpan().StartRef(), ix, asUpper.Length);
                var parsedLower = BitFuncParseRespCommandImpl.Calculate(ref respLowerBytes.AsSpan().EndRef(), ref respLowerBytes.AsSpan().StartRef(), ix, asLower.Length);
            ...
            foreach (var cmd in Enum.GetValues<RespCommand>())
            {
                ...
                for (var i = 0; i < asStr.Length; i++)
                {
                    for (var smashed = 0; smashed <= byte.MaxValue; smashed++)
                    {
                        ...
                        var parsed = BitFuncParseRespCommandImpl.Calculate(ref buffer.EndRef(), ref buffer.StartRef(), ix, asStr.Length);
